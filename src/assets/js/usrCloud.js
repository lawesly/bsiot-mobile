...
var UsrCloud = function () {
    ...
    //回调object
    var callbackobj = {
        USR_onSubAck: null,//弃用
        USR_onUnSubAck: null, //弃用
        USR_onRcv: null,//弃用
        USR_onConnAck: null,
        USR_onConnLost: null,
    ...
    var publish = function (topic, dataByte) {
        ...
        var message = new Paho.MQTT.Message(payloadBuff.buffer);
        message.qos = 1;
        message.retained = false;
        message.destinationName = topic;
        mqtt.send(message);
        !callbackobj.USR_onPubAck || callbackobj.USR_onPubAck(topic, dataByte);
        return 0;
    };
    this.USR_Publish=function (devId, dataByte) {
        let topic="$USR/DevRx/" + devId;
        publish(topic,dataByte);
    }
    /**
     * 向单台设备推送原始数据流
    ...
    var subscribe = function (topic, successFunc, failureFunc) {
        ...
        if (_.find(subscriptions, {'topic': topic})) {
            return 2;
        } else {
            mqtt.subscribe(topic, {
                ...
                onSuccess: function (response) {
                    ...
                    !callbackobj.USR_onSubAck || callbackobj.USR_onSubAck(response);
                },
    ...
    this.USR_Subscribe=function (devId) {
        let topic = "$USR/DevTx/" + devId;
        return subscribe(topic,function(){},function () {});
    }
    /**
     * 订阅设备原始数据流
     * @param devId
    ...
    var unSubscribe = function (topic, successFunc, failureFunc) {
        ...
        if (_.find(subscriptions, {'topic': topic})) {
            mqtt.unsubscribe(topic, {
                ...
                onSuccess: function (response) {
                    successFunc(response);
                    !callbackobj.USR_onUnSubAck || callbackobj.USR_onUnSubAck(response);
                },
                onFailure: failureFunc
            });
        } else {
    ...
    var USR_onUnSubscribeDevRawSucc = function (response) {
        subscriptions = _.filter(subscriptions, function (item) {
            return item.topic !== response.invocationContext.topic;
        });
    ...
    this.USR_UnSubScribeAll = function () {
        for (var key in subscriptions) {
...
stringToUTF8 = function (input, output, start) {
    var pos = start;
    for (var i = 0; i < input.length; i++) {
        var charCode = input.charCodeAt(i);
